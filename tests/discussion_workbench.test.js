...
function judgment(prepared,overrides={}){const symbol=prepared&&prepared.context&&prepared.context.symbol||'601138.SS';return {currentState:{symbol,sourceDiscussionVersion:prepared.sourceDiscussionVersion,userDecision:{headline:'可以继续持有，暂时没有明显减仓风险。',holding:{status:'safe',summary:'当前持有判断仍然稳定。'},positionDirection:{status:'hold_no_add',summary:'持有为主，暂不增加仓位。'},addAssessment:{status:'wait',summary:'等待更合适的机会，不追当前位置。'},warning:{summary:'若关键风险明显增强，需要重新复核当前判断。',items:[]},takeProfit:{status:'none',summary:'暂时没有明显止盈压力。'},stopLoss:{status:'none',summary:'关键支撑仍有效，暂时没有明显止损风险。'},riskSource:'none'},actionAssessment:{category:'hold_watch',priority:'low',headline:'当前没有临近的仓位决策窗口，维持常规观察。',reasons:['趋势修复但尚未出现需要调整仓位的确认信号。'],upgradeConditions:['关键结构确认后提高复核优先级。'],downgradeConditions:['当前修复结构被后续走势破坏。']},attentionLevel:'normal',trendAssessment:{overall:'recovery',timeframes:[{timeframe:'日线',status:'recovery',explanation:'价格延续修复但仍需量价确认。'}]},structureAssessment:[],stage:'修复观察',focusPoints:['观察量价能否确认修复延续。'],summary:'本轮确认继续观察。',keyChanges:['价格修复'],risks:['压力位风险'],watchPoints:['观察成交量'],planRelation:{status:'neutral',summary:'当前计划仅作为观察条件，不自动修改；价格触发不等于完整条件满足。'},confidence:'medium',...overrides}}}
...
test('Archive Prompt is short, strict, versioned, and does not resend history',()=>{
  ...
  assert.match(archive.request,/下划线 _ 不需要也不得转义/);assert.ok(archive.request.includes('discussion\\_v3\\_9a35cb46'));assert.match(archive.request,/只能使用标准定义的转义/);
