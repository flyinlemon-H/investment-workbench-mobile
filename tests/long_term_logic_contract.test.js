...
function logic(overrides={}){return {investmentThesis:'行业长期需求、公司交付护城河和组合成长角色共同支持继续跟踪。',coreDrivers:['行业长期需求','公司交付能力','组合成长角色'],keyRisks:['行业需求不及预期','公司竞争优势减弱'],reviewTriggers:['连续两个报告期需求下滑','交付优势不再成立'],logicStatus:'valid',confidence:'medium',nextReviewDate:'2026-12-01',...overrides}}
...
test('exact contract rejects unknown, missing, enum, type, date, and business-content violations with zero writes',async()=>{
  ...
  const missing=structuredClone(base);delete missing.longTermLogic.reviewTriggers;cases.push(missing);
  const badEnum=structuredClone(base);badEnum.longTermLogic.logicStatus='weakened';cases.push(badEnum);
  const wrongType=structuredClone(base);wrongType.longTermLogic.keyRisks='none';cases.push(wrongType);
  const badDate=structuredClone(base);badDate.longTermLogic.nextReviewDate='2026-02-30';cases.push(badDate);
  const dateOrder=structuredClone(base);dateOrder.longTermLogic.nextReviewDate='2026-08-01';cases.push(dateOrder);
  const shortTerm=structuredClone(base);shortTerm.longTermLogic.keyRisks=['跌破MA20'];cases.push(shortTerm);
  let writes=0;
...
test('successful candidate commits once, preserves protected domains, and creates current plus legacy history',async()=>{
  ...
  const next=adopted.stocks[0];assert.deepEqual(next.longTermLogic,{schemaVersion:Contract.MODULE_SCHEMA_VERSION,...logic()});assert.equal(next.longTermLogicAudit.current.responseHash,Contract.responseHash(result));assert.equal(next.longTermLogicAudit.history.length,1);assert.match(next.longTermLogicAudit.history[0].responseHash,/^legacy_/);
...
test('subsequent save moves the former current snapshot into bounded history consistently',async()=>{
  ...
  const nextStock=firstCandidate.stocks[0],secondPrepared=Workflow.prepare(nextStock,{promptDate:'2026-09-03'}),secondValue=envelope(secondPrepared.context);secondValue.longTermLogic={...logic(),nextReviewDate:'2026-12-02',investmentThesis:'新的行业长期需求、公司护城河和组合角色判断已经形成。'};
