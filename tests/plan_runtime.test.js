...
function market(){return {code:'600000.SS',shares:0,avgCost:0,type:'holding',priceHistory:[{date:'2026-09-03',close:10,is_complete_bar:true,adjustment:'qfq',price_basis:'close',provider:'fixture'}],technicalData:{technicalDataStatus:'fresh'}}}
function current(version='discussion_v1',stateId=`state_${version}`){
  const snapshot=Discussion.technicalSnapshot(market()),refs=Discussion.references(market());return Discussion.normalizeState({...{schemaVersion:Discussion.LEGACY_STATE_SCHEMA_VERSION,stateId,symbol:'600000.SS',sourceDiscussionVersion:version,stage:'结构观察',summary:'当前处于结构观察阶段，仍需等待进一步确认。',keyChanges:[],risks:['结构可能转弱'],watchPoints:['观察确认条件'],planRelation:'与观察纪律一致',confidence:'medium',technicalAsOf:'2026-09-03',confirmedAt:'2026-09-04T01:00:00.000Z',confirmedDate:'2026-09-04',technicalSnapshot:{anchorBar:{date:'2026-09-03',close:10,adjustment:'qfq',priceBasis:'close',provider:'fixture'}},references:{technical:{technicalAsOf:'2026-09-03',latestCompleteBar:'2026-09-03',anchorBar:{date:'2026-09-03',close:10,adjustment:'qfq',priceBasis:'close',provider:'fixture'}},plans:[],planReviews:[],holding:{},longTermLogic:{},modules:{}}},technicalSnapshot:snapshot,references:refs});
}
function stock(plans=[Plan.createWatchPlan(definition(),{now:'2026-09-04T00:00:00.000Z'})],discussion=current()){
  return {...market(),id:'stock_runtime',code:'600000.SS',name:'Runtime Fixture',type:'holding',shares:0,avgCost:0,capPct:20,plans,discussionState:{schemaVersion:Discussion.STORE_SCHEMA_VERSION,current:discussion,history:[]}};
}
