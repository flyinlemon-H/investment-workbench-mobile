...
function legacyImportRuntime(){
  ...
  vm.runInContext(`
    ...
    this.audit={longTerm:text=>{testElements.longTermLogicImportText.value=text;return importLongTermLogicJson()},longValue:()=>{const prepared=prepareLongTermLogic(state.stocks[0]);return {binding:{symbol:prepared.context.symbol,contextHash:prepared.context.contextHash},longTermLogic:{investmentThesis:'行业长期需求、公司交付护城河和组合成长角色共同支持继续跟踪。',coreDrivers:['行业长期需求','公司交付能力','组合成长角色'],keyRisks:['行业需求不及预期','公司竞争优势减弱'],reviewTriggers:['连续两个报告期需求下滑','交付优势不再成立'],logicStatus:'valid',confidence:'medium',nextReviewDate:'2026-12-01'}}},general:text=>importSentimentPayloadFromText(text),stock:()=>structuredClone(state.stocks[0]),calls:()=>structuredClone(saveCalls),alerts:()=>structuredClone(alerts),longPrompt:()=>longTermLogicPromptText(state.stocks[0]),ui:kind=>{const prefix=kind==='longTerm'?'longTermLogic':'sentiment';return {message:testElements[prefix+'ImportStatus'].textContent,disabled:testElements[prefix+'ImportSaveBtn'].disabled,alertClass:statusClasses.has('alert')}}};
  `,context);
  return context.audit;
}

test('Long-Term accepts strict, fenced, and structural-plus-content quote inputs without content corruption',async()=>{
  const seed=legacyImportRuntime(),payload=seed.longValue();payload.longTermLogic.investmentThesis='按“投资手册”方式整理长期判断。';payload.longTermLogic.coreDrivers=['行业处于“长期修复”阶段'];payload.longTermLogic.reviewTriggers=['若“核心需求”连续两个报告期下滑则复核'];
  for(const raw of [JSON.stringify(payload),`\`\`\`json\n${JSON.stringify(payload)}\n\`\`\``,smartStructure(payload)]){
    const app=legacyImportRuntime();await app.longTerm(raw);assert.equal(app.calls().length,1);assert.equal(app.stock().longTermLogic.investmentThesis,payload.longTermLogic.investmentThesis);assert.equal(app.stock().longTermLogic.reviewTriggers[0],payload.longTermLogic.reviewTriggers[0]);
  }
});
