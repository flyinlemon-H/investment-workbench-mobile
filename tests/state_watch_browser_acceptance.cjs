...
(async()=>{
  ...
  try{for(const viewport of [{width:1280,height:900},{width:390,height:844}]){
    ...
    assert.equal(facts.execution,0);assert.equal(facts.shares,0);assert.equal(facts.legacyName,'合成传统计划');assert.equal(facts.overflow,false);assert.deepEqual(facts.runtime,{schemaVersion:'plan-runtime.store.v1',byPlanId:{}});assert.equal(facts.runtimeFields,false);assert.deepEqual(errors,[]);assert.deepEqual(dialogs,[]);
