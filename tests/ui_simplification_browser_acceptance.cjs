...
(async()=>{
  ...
  try{for(const viewport of [{width:390,height:844},{width:360,height:800},{width:1280,height:900}]){
    ...
    fixture.stocks[0].managementCategory='core';fixture.stocks[1].managementCategory='candidate';fixture.stocks[2].managementCategory='etf';
    ...
    await auditUi('单股历史');
    await page.locator('[data-tab="targets"]').click();await page.locator('[data-target-filter="candidate"]').click();await page.locator('#targetSearch').fill('华勤');await page.locator('[data-target-stock="huaqin"]').first().click();assert.equal(await page.locator('#workspace-tab-ai').getAttribute('aria-selected'),'true');assert.match(await page.locator('.detail-compact-header').innerText(),/华勤技术/);
    await page.locator('[data-tab="targets"]').click();await page.locator('#targetSearch').fill('');for(const [filter,count] of [['core',1],['watch',0],['candidate',1],['etf',1]]){await page.locator(`[data-target-filter="${filter}"]`).click();assert.equal(await page.locator('.target-card').count(),count)}
    await auditUi('标的');
