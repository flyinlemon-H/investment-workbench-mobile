...
后续 CASE 4 暴露诊断误报：已经承认原条件满足、且价格随后明确延伸时，等待位置修复不应当被当作无理由提高原突破门槛。例外只接受 direction/confirmation/path 完整、position=extended、state=entry_extended、无否决证据且有 transitionReason 的结果；缺确认、位置可接受、未解释或仍 forming 均保留诊断。新增 5 项回归。模型偶发格式和状态映射不一致仍需如实记录，不通过放宽校验掩盖。
