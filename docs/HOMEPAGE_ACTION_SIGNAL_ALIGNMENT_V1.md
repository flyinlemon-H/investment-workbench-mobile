...
Validated source commit: `3e98240d970399fe6de5720d1ba98081b63a8e01`, based on the accepted daily-market commit `fe2aa04`. The following version commit only changes entrypoint cache bindings, manifest-generator release metadata, the existing asset-version assertion and this release record. Focused Homepage plus publication-boundary checks passed **74/74** after incorporating the generated-only baseline update. Full source validation remains 907 JavaScript / 21 Python / 12 browser scripts, with disclosed historical and transient outcomes above.

Version commit: `6d7b938030bc24b559f13b04578559c1c8e2edcc`. Generated manifest binds that committed source to `homepage-action-signal-alignment-v1-20260908`. Exact committed-source artifact validation (without recomputing away manifest mismatches) passes **81 source assets / 82 delivered files**, cache/dependency completeness and current market-bridge consistency. All **10 feature-related changed files** since fe2aa04 pass the credential scanner with zero findings. Release/version/publication tests pass **10/10**. Local output: ignored `test-results/action-release-artifact/`; final evidence: `test-results/action-release-validation.json`. The next commit records the manifest and this gate; final HEAD and ahead/behind are reported to the user. No push, Pages deployment, production smoke, real portfolio mutation or automatic AI call has been performed.
