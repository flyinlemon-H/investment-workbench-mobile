(function(root,factory){
  ...
  function legacyUnknownFields(source){
    const known=new Set(['id','schemaVersion','planVersion','action','type','planType','triggerPrice','price','triggerDirection','triggerOn','status','versionStatus','validityStatus','createdAt','updatedAt','lastReviewedAt','nextReviewDate','validUntil','priceTriggerStatus','priceCondition','triggeredAt','fullConditionStatus','conditions','reviewRequirement','invalidationReason','terminatedAt','archivedAt','source','note','summary','planSummary','actionHint','description','quantity','shares','allocationConstraint','maxPositionPct','maxWeight','targetWeightRange','legacy','stage','stockId','objectType']);
  ...
  function validatePlan(plan){
    const errors=[],source=object(plan);if(source.schemaVersion!==SCHEMA_VERSION)errors.push('schemaVersion 必须是 plan.v2');if(!text(source.id))errors.push('计划缺少 id');if(!Number.isInteger(source.planVersion)||source.planVersion<1)errors.push('planVersion 必须是正整数');if(!PLAN_STATUSES.includes(source.status))errors.push('计划状态无效');if(!VALIDITY_STATUSES.includes(source.validityStatus))errors.push('计划有效性状态无效');if(!PRICE_TRIGGER_STATUSES.includes(source.priceTriggerStatus))errors.push('价格触发状态无效');if(!FULL_CONDITION_STATUSES.includes(source.fullConditionStatus))errors.push('完整条件状态无效');if(source.triggerPrice!==null&&positive(source.triggerPrice)===null)errors.push('触发价格必须为正数或空');if(source.triggerDirection!==null&&!['above','below'].includes(source.triggerDirection))errors.push('触发方向无效');if(source.createdAt!==null&&!dateValue(source.createdAt))errors.push('创建时间无效');if(source.updatedAt!==null&&!dateValue(source.updatedAt))errors.push('更新时间无效');if(!SOURCES.includes(source.source))errors.push('计划来源无效');return {ok:errors.length===0,errors};
  }
  ...
  async function commitCandidate(currentState,buildCandidate,deps={}){
    ...
    const allPlans=stocks.flatMap(stock=>stock.plans||[]),validation=validatePlanCollection(allPlans);if(!validation.ok)return {status:'invalid',writes:0,error:new Error(validation.errors.join('；'))};
