(function decisionCompressionContractModule(root,factory){
  ...
  const ITEM_FIELDS=Object.freeze(['symbol','priority','actionCategory','reason','blockerPriority','planState','confidence']);
  const LEGACY_ITEM_FIELDS=Object.freeze(['symbol','priority','actionCategory','reason','blockers','planState','confidence']);
  const BLOCKER_PRECEDENCE=DecisionCompressionContext.BLOCKER_PRECEDENCE;
  const SAFETY_BLOCKERS=DecisionCompressionContext.SAFETY_BLOCKERS;
  ...
  const INTERNAL_TERMS=Object.freeze(['review_now','wait_confirmation','risk_control','watch','no_action','blockers','blockerPriority','blockerFacts','PlanReview','freshness','validityStatus','priceTriggerStatus','fullConditionStatus','planVersion','historical_only','likely_invalid']);
  ...
  function blockerFacts(stockFacts){
    const explicit=array(stockFacts&&stockFacts.blockerFacts).filter(code=>BLOCKERS.includes(code));
    if(explicit.length)return BLOCKER_PRECEDENCE.filter(code=>explicit.includes(code));
    const eligibility=object(stockFacts&&stockFacts.eligibleBlockers)?stockFacts.eligibleBlockers:{};
    return BLOCKER_PRECEDENCE.filter(code=>eligibility[code]===true);
  }
  function displayBlockers(item){
    const facts=array(item&&item.blockerFacts).length?array(item.blockerFacts):array(item&&item.blockers),emphasis=array(item&&item.blockerPriority),out=[];
    const add=code=>{if(facts.includes(code)&&!out.includes(code))out.push(code)};
    BLOCKER_PRECEDENCE.filter(code=>SAFETY_BLOCKERS.includes(code)).forEach(add);
    emphasis.forEach(add);
    BLOCKER_PRECEDENCE.forEach(add);
    facts.forEach(add);
  ...
  function validate(value,options={}){
    ...
    const sourceDecision=value.decisionCompression;if(!object(sourceDecision))return invalid('invalid_decision','decisionCompression 必须是对象。');const shape=exactFields(sourceDecision,ROOT_FIELDS,'decisionCompression');if(shape)return invalid('invalid_decision',shape);const decision=clone(sourceDecision);
    ...
    for(let index=0;index<decision.items.length;index+=1){
      const item=decision.items[index],path=`items[${index}]`;if(!object(item))return invalid('invalid_item',`${path} 必须是对象。`);const hasPriority=Object.prototype.hasOwnProperty.call(item,'blockerPriority'),hasLegacy=Object.prototype.hasOwnProperty.call(item,'blockers');if(hasPriority&&hasLegacy)return invalid('invalid_item',`${path} 不能同时包含 blockerPriority 与旧 blockers。`);const itemShape=exactFields(item,hasLegacy?LEGACY_ITEM_FIELDS:ITEM_FIELDS,path);if(itemShape)return invalid('invalid_item',itemShape);
      ...
      const stockFacts=facts.get(symbol),eligibility=object(stockFacts&&stockFacts.eligibleBlockers)?stockFacts.eligibleBlockers:{},emphasis=hasLegacy?item.blockers:item.blockerPriority;if(!Array.isArray(emphasis)||emphasis.length>2||new Set(emphasis).size!==emphasis.length)return invalid('invalid_blockers',`${symbol} blockerPriority 必须是最多 2 个且不重复。`);for(const blocker of emphasis){if(!BLOCKERS.includes(blocker))return invalid('invalid_blocker',`${symbol} blocker 引用无效：${text(blocker)}。`);if(eligibility[blocker]!==true)return invalid('unsupported_blocker',`${symbol} blocker 引用没有程序事实支持：${blocker}。`)}item.blockerPriority=emphasis.slice();delete item.blockers;item.blockerFacts=blockerFacts(stockFacts);
      ...
      if(item.planState==='likely_invalid_unconfirmed'&&item.actionCategory!=='review_now')return invalid('unsafe_likely_invalid',`${symbol} 的可能失效判断必须保持为立即复核。`);
    }
    for(let index=0;index<decision.noActionSymbols.length;index+=1){const raw=decision.noActionSymbols[index];if(typeof raw!=='string')return invalid('invalid_no_action',`noActionSymbols[${index}] 必须是字符串。`);const symbol=canonical(raw);if(!symbol||!allowed.has(symbol))return invalid('unknown_symbol',`noActionSymbols[${index}] 不在本次所选股票中：${text(raw)||'（空）'}。`);if(seen.has(symbol))return invalid('duplicate_symbol',`symbol 重复：${symbol}。`);const stockFacts=facts.get(symbol);if(stockFacts&&stockFacts.highAttentionEligible===true)return invalid('material_symbol_in_no_action',`${symbol} 存在需要详细处理的当前事实，不能仅放入暂不处理。`);seen.add(symbol);decision.noActionSymbols[index]=symbol}
  ...
  function renderPreview(result,nameForSymbol=symbol=>symbol){const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));if(!result||!result.ok)return `<div class="hint"><b>结果校验失败</b><div class="card-note">${esc(result&&result.message||'无法校验结果。')}</div><div class="card-note">尚未写入任何数据。</div></div>`;const action={review_now:'立即复核',wait_confirmation:'等待确认',risk_control:'风险控制',watch:'观察'},priority={high:'高',medium:'中',low:'低'};return `<div class="hint"><b>预览通过</b><div class="card-note">${result.decision.items.length} 项详细处理 · ${result.decision.noActionSymbols.length} 只已选但暂不处理 · 尚未写入</div></div><div class="card"><div class="card-title">今日组合结论</div><div>${esc(result.decision.overallSummary)}</div></div>${result.decision.items.map(item=>`<div class="card"><div class="card-title">${esc(nameForSymbol(item.symbol))} · ${esc(action[item.actionCategory])} · ${esc(priority[item.priority])}</div><div>${esc(item.reason)}</div></div>`).join('')}`}
