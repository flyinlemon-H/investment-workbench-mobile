(function portfolioReviewContextModule(root,factory){
  ...
  function planDirection(plan){const explicit=text(plan&&plan.triggerOn||plan&&plan.triggerDirection).toLowerCase();if(['above','below'].includes(explicit))return explicit;return /sell|reduce|trim/.test(text(plan&&plan.action||plan&&plan.type).toLowerCase())?'above':'below'}
  function compactPlans(stock){
    const price=currentPrice(stock);
    return arr(stock&&stock.plans).filter(plan=>{const status=text(plan&&plan.status).toLowerCase();return !['completed','cancelled','canceled','closed','done','archived','expired'].includes(status)}).map(plan=>{
      const reviewRequirement=object(plan&&plan.reviewRequirement),triggerPrice=positive(plan&&((plan.price!==undefined)?plan.price:plan.triggerPrice)),triggerOn=planDirection(plan),crossed=Boolean(price&&triggerPrice&&((triggerOn==='below'&&price<=triggerPrice)||(triggerOn==='above'&&price>=triggerPrice))),conditions=[...arr(reviewRequirement.conditions).map(text),text(plan&&plan.condition),text(plan&&plan.technicalCondition),text(plan&&plan.volumeCondition),text(plan&&plan.allocationCondition),text(plan&&plan.riskControl)].filter(Boolean);
      return {id:text(plan&&plan.id),action:text(plan&&plan.action||plan&&plan.type)||'watch',triggerPrice,triggerOn,status:text(plan&&plan.status)||'active',priceCondition:triggerPrice===null?'not_available':(crossed?'triggered':'not_triggered'),additionalConditions:conditions.slice(0,6),requiresManualConfirmation:true,fullConditionStatus:'unproven',userMeaning:crossed?'价格已触发，待确认其他条件':'尚未达到价格条件',note:text(plan&&plan.summary||plan&&plan.actionHint||plan&&plan.note||plan&&plan.description)};
    }).filter(plan=>plan.triggerPrice!==null||plan.note).slice(0,8);
  }
  function stockContext(stock,options={}){const symbol=canonicalSymbol(symbolOf(stock));if(!symbol)throw new Error('组合复核股票缺少 canonical symbol。');return {stock:{name:text(stock&&stock.name),symbol,role:text(stock&&stock.role),theme:text(stock&&stock.theme),type:text(stock&&stock.type)},holding:holdingFacts(stock,options.portfolioMarketValue),allocation:compactAllocation(stock),technical:compactTechnical(stock,options),news:compactNews(stock,options),fundamental:compactFundamental(stock,options),valuation:compactValuation(stock,options),longTermLogic:compactLongTerm(stock,options),plans:compactPlans(stock)}}
  ...
  function readiness(stocks){
    const counts=Object.fromEntries(['technical','news','fundamental','valuation','longTermLogic'].map(key=>[key,relevanceCounts(stocks,key)])),highImpactProblems=['technical','news'].reduce((sum,key)=>sum+counts[key].outdated+counts[key].unavailable+counts[key].inconsistent,0);
    return {stockCount:stocks.length,technical:readinessLabel(counts.technical),news:readinessLabel(counts.news),fundamental:readinessLabel(counts.fundamental),valuation:readinessLabel(counts.valuation),longTermLogic:readinessLabel(counts.longTermLogic),counts,suggestedConfidence:highImpactProblems>=2?'low':(highImpactProblems===1?'medium':'high')};
  }
  ...
  function buildPortfolioContext(stocks,options={}){
    ...
    return {reviewDate,generatedAt:text(options.generatedAt)||new Date().toISOString(),portfolio,readiness:readiness(contexts),coordinationLimitations:coordinationLimitations(contexts,portfolio),stocks:contexts};
  }
  function buildRequest(stocks,options={}){
    ...
    return [
      ...
      '事实协调规则：','1. 只能使用下方结构化事实；程序拥有当前持仓、价格、日期、计划、todayRelevance 和数据一致性，禁止重算或改写。','2. 当前持仓事实优先于配置分析中的历史仓位描述；当前程序技术事实优先于旧技术叙述；当前新闻快照优先于历史催化；当前存储计划优先于旧评论。','3. 只有 todayRelevance=current 的高时效资料可直接描述为今日信息；usable_with_caution 只能作为带日期的谨慎参考；outdated 只能作为历史背景；stale、unavailable、unknown 或 inconsistent 都必须降低对应事实的可用性。','4. currentSnapshot 与 historicalReference 不得混用；历史新闻只能称为“历史参考”或“延续性背景”。','5. 实际券商持仓、成交和订单具有最终权威；本次选择的股票不一定等于完整券商组合。','6. 明确区分 held、watchlist 与 zero_position_candidate；零仓候选不得使用“继续持有”等措辞。','7. 复核既有计划，但不得修改、覆盖或新增存储计划。价格跨过阈值只表示“价格已触发，待确认其他条件”，绝不等于完整计划条件已满足。','8. 不得发明持仓、价格、财务、新闻、计划、仓位或市场背景，也不给确定性买卖指令。','',
