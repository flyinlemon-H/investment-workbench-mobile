(function portfolioReviewContextModule(root,factory){
  ...
  const api=factory(identity,planV2);
  ...
  function compactPlans(stock,options={}){
    const price=currentPrice(stock),reviewDate=options.reviewDate;
    return arr(stock&&stock.plans).map(plan=>PlanV2.normalizePlan(plan)).filter(plan=>plan.status==='active'&&['active','needs_review'].includes(plan.validityStatus)).map(plan=>{
      const compact=PlanV2.compactForPortfolio(plan,price,reviewDate),additionalConditions=Object.values(compact.conditions).flat().map(item=>item.text).filter(Boolean).slice(0,8);
      return {...compact,triggerOn:compact.triggerDirection,status:'active',priceCondition:compact.priceTriggerStatus==='unavailable'?'not_available':compact.priceTriggerStatus,additionalConditions,requiresManualConfirmation:true};
  ...
  function buildRequest(stocks,options={}){
    ...
    return [
      ...
      '事实协调规则：','1. 只能使用下方结构化事实；程序拥有当前持仓、价格、日期、计划、todayRelevance 和数据一致性，禁止重算或改写。','2. 当前持仓事实优先于配置分析中的历史仓位描述；当前程序技术事实优先于旧技术叙述；当前新闻快照优先于历史催化；只有标记为当前有效的计划可作为当前计划输入。','3. 只有 todayRelevance=current 的高时效资料可直接描述为今日信息；usable_with_caution 只能作为带日期的谨慎参考；outdated 只能作为历史背景；stale、unavailable、unknown 或 inconsistent 都必须降低对应事实的可用性。','4. currentSnapshot 与 historicalReference 不得混用；历史新闻只能称为“历史参考”或“延续性背景”。','5. 实际券商持仓、成交和订单具有最终权威；本次选择的股票不一定等于完整券商组合。','6. 明确区分 held、watchlist 与 zero_position_candidate；零仓候选不得使用“继续持有”等措辞。','7. 复核既有计划，但不得修改、覆盖或新增存储计划。价格跨过阈值只表示“价格已触发，待确认其他条件”，绝不等于完整计划条件已满足。需复核或历史参考计划不得静默提高今日优先级。','8. 不得发明持仓、价格、财务、新闻、计划、仓位或市场背景，也不给确定性买卖指令。','',
