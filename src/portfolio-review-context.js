(function portfolioReviewContextModule(root,factory){
  ...
  function compactLongTerm(stock,options={}){
    const logic=object(stock&&stock.longTermLogic),complete=object(stock&&stock.informationCompleteness),thesis=text(logic.investmentThesis||stock&&stock.thesis||stock&&stock.notes),auditDate=stock&&stock.longTermLogicAudit&&stock.longTermLogicAudit.current&&stock.longTermLogicAudit.current.updatedAt,risks=logic.keyRisks||logic.longTermRisks,available=Boolean(thesis||logic.updatedAt||auditDate||arr(logic.coreDrivers).length||arr(risks).length),status=available?(text(logic.logicStatus)||'unclear'):'unavailable',analysisDate=dateOnly(logic.updatedAt||auditDate),validUntil=dateOnly(logic.validUntil),nextReviewDate=dateOnly(logic.nextReviewDate);
    ...
    return {status,analysisDate,validUntil,nextReviewDate,todayRelevance,dataQuality:todayRelevance==='unavailable'?'unavailable':'consistent',confidence:text(logic.confidence)||'low',thesisSummary:thesis,validationPoints:arr(logic.coreDrivers).map(text).filter(Boolean).slice(0,5),invalidationRisks:arr(risks).map(text).filter(Boolean).slice(0,5),dataCompleteness:text(complete.longTermLogic)||'unknown'};
  }
