(function(root){
  'use strict';
  const D=root.DiscussionV4,C=root.PlanContextContract,esc=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let busy=false;
  function decisionPanel(stock){
    const source=D.sourceFor(stock.id),latest=D.latestDecision(state,root.DiscussionWorkbench.canonical(stock)),check=D.revalidateSource(state,stock.id,source);
    return `<section class="card" data-v4-decision-panel><div class="card-title">进入原因</div><p>${source?source.type==='homepage'?'从今日关注进入':source.type==='plan'?'复核此项长期计划':'手动开始讨论':'手动进入标的'}</p>${source?`<div class="card-note">进入时资料日期：${esc(source.sourceAsOf||'未提供')}。${check.status==='changed'?'进入时的条件已变化；当前事实已重新核对，请据此讨论。':'开始讨论时仍会重新核对当前事实。'}</div>`:''}<div class="card-title">我的确认</div><p class="card-note">上方是 AI 当前判断；下方记录你自己的选择，不执行交易。</p>${latest?`<p data-v4-latest-decision><b>${esc(D.LABELS[latest.outcome])}</b> · ${esc(latest.confirmedAt.slice(0,16).replace('T',' '))}<br>${esc(latest.note)}</p>`:'<p class="card-note">尚未记录你的确认。</p>'}<label>本次选择<select id="v4DecisionOutcome">${D.OUTCOMES.filter(o=>stock.shares>0?o!=='consider_entry':!['consider_increase','consider_reduce'].includes(o)).map(o=>`<option value="${o}"${o==='continue_waiting'?' selected':''}>${esc(D.LABELS[o])}</option>`).join('')}</select></label><label>补充说明（可选）<textarea id="v4DecisionNote" maxlength="500" rows="2"></textarea></label><button class="btn" type="button" data-v4-confirm-decision data-stock="${esc(stock.id)}">确认我的选择</button><div id="v4DecisionStatus" role="status" class="card-note"></div></section>`;
  }
  async function confirmDecision(stockId){
    if(busy)return;const box=document.getElementById('v4DecisionStatus');busy=true;
    try{const prepared=D.prepareDecision(state,stockId,{outcome:document.getElementById('v4DecisionOutcome').value,note:document.getElementById('v4DecisionNote').value});const result=await D.commitDecision(state,prepared,{saveCandidate:persist,adoptCandidate:next=>{state=next}},{confirmed:true});if(result.status!=='completed')throw result.error||new Error('未保存');renderStockDetail()}
    catch(error){box.textContent=error.message}finally{busy=false}
  }
  document.addEventListener('click',event=>{const button=event.target.closest('[data-v4-confirm-decision]');if(button)confirmDecision(button.dataset.stock)});
  ...
  function auditPanel(stock){
    ...
    return `<section class="card" data-v4-audit-panel><h3>我的确认与计划变更记录</h3>${decisions.length?decisions.map(d=>{const discussion=s.discussions[d.discussionId];return `<details id="v4-audit-${esc(d.decisionId)}"><summary>${esc(D.LABELS[d.outcome])} · ${esc(d.confirmedAt)}</summary><p>进入原因：${esc(discussion.sourceContext?.reasonCode||'手动讨论')}</p><p>当时 AI 判断：${esc(discussion.judgment?.summary||'未提供 AI 判断')}</p><p>我的说明：${esc(d.note||'无补充')}</p>${receipts.filter(r=>r.source.decisionId===d.decisionId).map(r=>`<article id="v4-audit-${esc(r.receiptId)}"><h4>${esc(operationLabels[r.operation])}</h4><p>确认时间：${esc(r.confirmedAt)}</p><p>原因：${esc(r.draft.reason)}</p><p style="overflow-wrap:anywhere">原计划：${esc(r.priorPlanRef?.planId||'无')}<br>结果计划：${esc(r.newPlanRef?.planId||'无')}<br>修订：${esc(r.newPlanRef?.definitionRef?.revisionId||'保留旧计划格式')}</p><details><summary>查看当时定义与完整来源</summary><pre style="white-space:pre-wrap;overflow-wrap:anywhere">${esc(JSON.stringify(r,null,2))}</pre></details></article>`).join('')||'<p>没有确认长期计划变更。</p>'}</details>`}).join(''):'<p>尚无用户确认记录。</p>'}</section>`;
  }
  document.addEventListener('click',async event=>{
    const button=event.target.closest('[data-v4-prepare-plan],[data-v4-preview-plan],[data-v4-confirm-plan],[data-v4-discuss-plan],[data-v4-audit]');if(!button||busy)return;
    const stockId=button.dataset.stock,stock=state.stocks.find(s=>s.id===stockId),W=root.DiscussionPlanWorkflow,box=document.getElementById('v4PlanStatus');
    try{
  ...
  root.PlanDiscussionV4UI={decisionPanel,planPanel,auditPanel};
})(globalThis);
