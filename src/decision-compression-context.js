(function decisionCompressionContextModule(root,factory){
  ...
  function buildRequest(portfolioSnapshot,stocks,options={}){
    const context=buildDecisionContext(portfolioSnapshot,stocks,options),first=context.stocks[0],rest=context.stocks.slice(1).map(item=>item.symbol),example={decisionCompression:{reviewDate:context.reviewDate,overallSummary:'今天先复核最关键的计划与风险冲突，其余标的保持观察。',items:first?[{symbol:first.symbol,priority:first.highAttentionEligible?'high':'medium',actionCategory:first.highAttentionEligible?'review_now':'watch',reason:'用一句自然中文说明最主要的处理原因。',blockers:Object.keys(first.eligibleBlockers).filter(key=>first.eligibleBlockers[key]).slice(0,1),planState:first.allowedPlanStates[0],confidence:first.technical.todayUse==='current'?'medium':'low'}]:[],noActionSymbols:rest,confidence:'medium',limitations:['当前没有权威市场环境判断，相关结论保持谨慎。']}};
    return [
      '你是谨慎的每日组合决策压缩助理。你的任务是回答“现在最值得用户注意什么”，不是给出确定性交易指令。','',
      '权威边界：','1. 程序拥有持仓、权重、计划有效性、价格触发、完整条件、计划复核新鲜度、资料适用性和组合复核来源状态；不得重算、改写或发明。','2. AI 只判断主导原因、实际复核优先级、阻碍条件的重要性和处理类型。用户确认后才能修改计划；本任务不得修改计划、计划复核、持仓、配置或订单。','3. 价格已触发只表示“计划价格已触发，但完整条件尚未确认”，绝不等于可以直接执行。','4. needs_review 或 likely_invalid 只能提高复核优先级，不能把正式计划标记失效；historical_only 仅作历史参考，不能单独驱动高优先级。','5. staleReview 只说明旧复核已过期；不得采用其旧结论。计划变化后应写“计划变更后尚未重新复核”。','6. 当前没有权威市场环境或盘中模块。不得声称大盘突破、牛市开始、市场转强、风险偏好提升，也不得发明盘中涨跌、跳空、午后反转或实时量价。','',
      '压缩规则：','1. items 只保留 3–5 个最高价值事项；所选股票少于 3 只或实际事项不足时可以更少，但最多 5 个。其余股票放入 noActionSymbols，不生成等大的详细卡。','2. priority 只表示注意力优先级：high 今天需要复核；medium 值得监控；low 低优先。不得因为存在计划就机械标 high。','3. actionCategory 只能为 review_now, wait_confirmation, risk_control, watch, no_action；它们分别表示立即复核、等待确认、风险控制、观察、暂不处理，不是买卖指令。no_action 应放入 noActionSymbols。','4. 每个 reason 只写一句自然中文，保留一至两个主导驱动，不罗列全部模块。blockers 最多 2 个且只能从该股票 eligibleBlockers 中选择。','5. planState 只能从该股票 allowedPlanStates 中选择。likely_invalid_unconfirmed 要表达“可能已不适用，但尚未由用户确认失效”。','6. 缺失资料只有在 decisionRelevant 或对应 eligibleBlockers=true 时才进入首层；不得让数据限制压过主导决策原因。','7. 技术资料非 current 时，该股票 confidence 不得为 high。','',
      '危险措辞：','压缩文字中禁止出现“立即买入、立即卖出、必须加仓、必须减仓、直接执行计划、满仓、清仓”。不得输出内部字段名、英文枚举或 PlanReview 等实现术语。','',
      '严格 JSON 契约：','1. 只输出严格 JSON；顶层只能包含 decisionCompression。','2. decisionCompression 必须完整包含 reviewDate, overallSummary, items, noActionSymbols, confidence, limitations。','3. item 必须完整包含 symbol, priority, actionCategory, reason, blockers, planState, confidence。','4. symbol 必须精确来自 selectedSymbols；items 与 noActionSymbols 合计必须恰好覆盖全部 selectedSymbols，不得缺少、重复或新增。','5. items 必须按 high、medium、low 排序，最多 5 项。blockers 最多 2 项。limitations 最多 3 项。','6. reviewDate 必须与输入一致；priority 只能为 high, medium, low；confidence 只能为 high, medium, low。','',
  ...
  return Object.freeze({SCHEMA_VERSION,MAX_DETAILED_ITEMS,MARKET_CONTEXT,portfolioReviewReference,reviewSignals,allocationConflict,allowedPlanStates,buildDecisionContext,buildRequest,requestMetrics,priorityRank,clone});
});
