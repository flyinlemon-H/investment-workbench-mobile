(function(root){
  ...
  function card(plan,stock,appState){
    ...
    const assessment=Runtime.assessmentStatus(appState,plan.id),stale=status!=='current'||assessment.bindingStatus!=='current'||assessment.evidenceFreshness.status!=='current'||assessment.applicability.status!=='applicable',warning=status==='definition_changed'?'计划定义已变化，需要重新复核当前状态。':status==='current_state_changed'?'最新个股结论已变化，需要重新复核当前状态。':status==='missing_plan'?'原计划已不存在；仅保留历史状态记录。':'';
