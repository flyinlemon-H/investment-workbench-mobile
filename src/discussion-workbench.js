(function(root,factory){
  ...
  const V2_STATE_SCHEMA_VERSION='stock-discussion.state.v2';
  const STATE_SCHEMA_VERSION='stock-discussion.state.v3';
  const CONTEXT_SCHEMA_VERSION='stock-discussion.context.v3';
  ...
  const HOLDING_STATUSES=Object.freeze(['safe','caution','reduce_review','risk_control','not_applicable']);
  const POSITION_DIRECTION_STATUSES=Object.freeze(['hold','hold_no_add','add_watch','add_review','reduce_review','risk_control','not_applicable']);
  const ADD_ASSESSMENT_STATUSES=Object.freeze(['wait','watch','add_review','avoid','not_applicable']);
  const TAKE_PROFIT_STATUSES=Object.freeze(['none','watch','review','not_applicable']);
  const STOP_LOSS_STATUSES=Object.freeze(['none','watch','risk_control','not_applicable']);
  const RISK_SOURCES=Object.freeze(['none','stock','market','both','unclear']);
  ...
  function normalizeReferences(value){
    ...
    return {
      ...
      longTermLogic:{updatedAt:dateValue(source.longTermLogic&&source.longTermLogic.updatedAt||source.longTermLogicAudit&&source.longTermLogicAudit.current&&source.longTermLogicAudit.current.updatedAt),logicStatus:text(source.longTermLogic&&source.longTermLogic.logicStatus),hash:text(source.longTermLogic&&source.longTermLogic.hash)},
  ...
  function normalizeUserDecision(value){
    const source=object(value),section=key=>{const item=object(source[key]);return {status:text(item.status),summary:text(item.summary)}};
    return {headline:text(source.headline),holding:section('holding'),positionDirection:section('positionDirection'),addAssessment:section('addAssessment'),warning:{summary:text(source.warning&&source.warning.summary),items:uniqueStrings(source.warning&&source.warning.items,3)},takeProfit:section('takeProfit'),stopLoss:section('stopLoss'),riskSource:text(source.riskSource)};
  }
  ...
  function normalizeState(value){
    ...
    return {
      schemaVersion:source.schemaVersion===V2_STATE_SCHEMA_VERSION?V2_STATE_SCHEMA_VERSION:STATE_SCHEMA_VERSION,stateId:text(source.stateId),symbol:canonical(source.symbol),sourceDiscussionVersion:text(source.sourceDiscussionVersion),...(source.schemaVersion===V2_STATE_SCHEMA_VERSION?{}:{userDecision:normalizeUserDecision(source.userDecision)}),actionAssessment:normalizeActionAssessment(source.actionAssessment),attentionLevel:text(source.attentionLevel),trendAssessment:normalizeTrendAssessment(source.trendAssessment),structureAssessment:normalizeStructureAssessment(source.structureAssessment),stage:text(source.stage),focusPoints:uniqueStrings(source.focusPoints,5),summary:text(source.summary),keyChanges:uniqueStrings(source.keyChanges,5),risks:uniqueStrings(source.risks,5),watchPoints:uniqueStrings(source.watchPoints,5),planRelation:normalizePlanRelation(source.planRelation),confidence:CONFIDENCE_LEVELS.includes(text(source.confidence))?text(source.confidence):'low',technicalAsOf:validDate(source.technicalAsOf),confirmedAt:dateValue(source.confirmedAt),confirmedDate:validDate(source.confirmedDate),technicalSnapshot:normalizeTechnicalSnapshot(source.technicalSnapshot),references:normalizeReferences(source.references)
    };
  }
  ...
  function validateUserDecision(source,errors){
    const decision=object(source),section=(key,statuses)=>{const value=object(decision[key]);exactFields(decision[key],['status','summary'],`userDecision.${key}`,errors);if(!statuses.includes(text(value.status)))errors.push(`userDecision.${key}.status invalid`);if(!text(value.summary)||text(value.summary).length>160)errors.push(`userDecision.${key}.summary invalid`)};
    exactFields(source,['headline','holding','positionDirection','addAssessment','warning','takeProfit','stopLoss','riskSource'],'userDecision',errors);
    if(!text(decision.headline)||text(decision.headline).length>120||/[\r\n]/.test(text(decision.headline)))errors.push('userDecision.headline invalid');
    section('holding',HOLDING_STATUSES);section('positionDirection',POSITION_DIRECTION_STATUSES);section('addAssessment',ADD_ASSESSMENT_STATUSES);
    exactFields(decision.warning,['summary','items'],'userDecision.warning',errors);if(!text(decision.warning&&decision.warning.summary)||text(decision.warning&&decision.warning.summary).length>160)errors.push('userDecision.warning.summary invalid');validStringList(object(decision.warning),'items',3,160,errors);
    section('takeProfit',TAKE_PROFIT_STATUSES);section('stopLoss',STOP_LOSS_STATUSES);
    if(!RISK_SOURCES.includes(text(decision.riskSource)))errors.push('userDecision.riskSource invalid');
  }
  function validateState(value){
    ...
    const isV2=source.schemaVersion===V2_STATE_SCHEMA_VERSION,allowed=['schemaVersion','stateId','symbol','sourceDiscussionVersion',...(isV2?[]:['userDecision']),'actionAssessment','attentionLevel','trendAssessment','structureAssessment','stage','focusPoints','summary','keyChanges','risks','watchPoints','planRelation','confidence','technicalAsOf','confirmedAt','confirmedDate','technicalSnapshot','references'];
    const extra=Object.keys(source).filter(key=>!allowed.includes(key));
    if(extra.length)errors.push(`state contains unknown fields: ${extra.join(', ')}`);
    if(![V2_STATE_SCHEMA_VERSION,STATE_SCHEMA_VERSION].includes(source.schemaVersion))errors.push('discussion state schema invalid');
    for(const key of ['stateId','symbol','sourceDiscussionVersion','stage','summary','confirmedAt','confirmedDate'])if(!state[key])errors.push(`${key} missing`);
    if(state.stage.length>40||/[\r\n]/.test(state.stage))errors.push('stage invalid');
    ...
    if(!CONFIDENCE_LEVELS.includes(text(source.confidence)))errors.push('confidence invalid');
    if(!isV2)validateUserDecision(source.userDecision,errors);
    if(!state.technicalAsOf||state.technicalAsOf!==state.references.technical.technicalAsOf)errors.push('technical anchor mismatch');
    const anchor=state.technicalSnapshot.anchorBar;
    if(!anchor.date||!(anchor.close>0)||anchor.date!==state.technicalAsOf)errors.push('technical anchor bar invalid');
    if(state.symbol!==canonical(source.symbol))errors.push('symbol invalid');
  ...
  function references(stock,options={}){
    const technical=technicalSnapshot(stock),plans=planReferences(stock,options),holdingCore={shares:number(stock&&stock.shares),avgCost:number(stock&&stock.avgCost||stock&&stock.cost),role:text(stock&&stock.role),type:text(stock&&stock.type)},logic=object(stock&&stock.longTermLogic),logicCore={updatedAt:dateValue(logic.updatedAt||stock&&stock.longTermLogicAudit&&stock.longTermLogicAudit.current&&stock.longTermLogicAudit.current.updatedAt),logicStatus:text(logic.logicStatus),investmentThesis:text(logic.investmentThesis||stock&&stock.thesis),coreDrivers:uniqueStrings(logic.coreDrivers,8),longTermRisks:uniqueStrings(logic.keyRisks||logic.longTermRisks,8)};
