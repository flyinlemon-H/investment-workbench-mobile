(function(root){
  'use strict';
  let activePreview=null,lastUpdates=[],generation=0;
  ...
  function close(){generation++;const modal=document.getElementById('analysisSyncDialog');if(modal)modal.classList.remove('show');activePreview=null}
  ...
  async function openPublish(stockId){
    if(!root.ManualAnalysisSyncCloud.publisherUi())return;
    activePreview=null;const requestGeneration=++generation;
  ...
  async function fetchUpdates(){
    ...
    try{
      const result=await engine.fetchUpdates(state);if(requestGeneration!==generation)return;lastUpdates=result.updates;
      if(!lastUpdates.length){show('获取分析更新',`<div class="hint">已是最新${result.unsupported.length?'；另有当前版本暂不支持的更新':''}。</div>`);return}
      const rows=lastUpdates.map((item,index)=>`<button class="analysis-update-item" type="button" data-analysis-update="${index}" ${item.eligibility?.ok?'':'disabled'}><b>${h(item.label)}</b><span>PC 更新：${h(new Date(item.envelope.publishedAt).toLocaleString('zh-CN'))}</span><span>${item.eligibility?.ok?'查看差异 →':h(item.eligibility?.message||'本机暂不能应用此更新。')}</span></button>`).join('');show(`发现 ${lastUpdates.length} 项更新`,rows);document.querySelectorAll('[data-analysis-update]').forEach(button=>button.onclick=()=>openApply(Number(button.dataset.analysisUpdate)));
  ...
  async function confirmActive(){
    if(!activePreview)return;if(activePreview.direction==='publish'&&!root.ManualAnalysisSyncCloud.publisherUi()){close();return}const requestGeneration=generation,preview=activePreview,button=document.getElementById('analysisSyncConfirm'),message=document.getElementById('analysisSyncMessage');button.disabled=true;message.textContent='正在确认精确版本…';
    try{
      ...
      if(preview.direction==='publish'){
        ...
        root.ManualAnalysisSyncCloud.mark('long_term_logic',entityKey,'verification_failed',result.message||'同步尚未确认');activePreview=null;button.hidden=true;message.textContent=result.message||'版本已变化，请重新预览。';return;
      }
  ...
  function initializeUi(){const button=document.getElementById('analysisFetchBtn');if(!button||!root.ManualAnalysisSyncCloud)return;root.ManualAnalysisSyncCloud.initialize();button.hidden=!root.ManualAnalysisSyncCloud.mobileUi();button.onclick=fetchUpdates}
  root.invalidateAnalysisSyncPreview=()=>{generation++;activePreview=null;lastUpdates=[];const button=document.getElementById('analysisSyncConfirm');if(button)button.hidden=true};
