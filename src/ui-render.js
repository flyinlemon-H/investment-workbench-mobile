...
const DETAIL_WORKSPACE_TABS=Object.freeze(['ai','plan','operation','technical','news','fundamental','valuation','longterm','research','history']);
const DETAIL_WORKSPACE_SESSION_KEY='v13_detail_workspace_tab_v1';
function normalizeDetailWorkspace(value){return DETAIL_WORKSPACE_TABS.includes(String(value||''))?String(value):'ai'}
...
function renderSyncHint(){
  ...
  el.textContent=showDiagnostics?[universeText,local,backend].filter(Boolean).join(' · '):'';
  el.hidden=!showDiagnostics;
...
function updateChecklistRows(){
  // Compatibility only: homepage attention is derived by HomepageAttention.
  return [];
}
...
function v13PlanDisplayCategory(plan){
  if(!PlanV2.isLegacyPricePlan(plan))return 'observe';
...
function v13PlanTriggerText(plan){
  if(!PlanV2.isLegacyPricePlan(plan))return esc(plan.name||'状态观察计划');
...
function v13PlanQuantityText(plan){
  if(!PlanV2.isLegacyPricePlan(plan))return PlanV2.REVIEW_ACTION_LABELS[plan.reviewAction]||'暂不支持在此处编辑或执行';
...
function v13PlanGapInfo(stock,plan){
  if(!PlanV2.isLegacyPricePlan(plan))return null;
...
function v13DerivedPlanValidity(stock,plan){
  if(!PlanV2.isLegacyPricePlan(plan))return {status:'unavailable',label:'只读',reason:'状态观察计划暂不支持在此处编辑或执行',gap:null,priceTriggered:false,refreshNeeded:false,homeEligible:false,reviewRequired:false,freshness:PlanV2.freshness(plan,todayDate()),plan:PlanV2.normalizePlan(plan)};
...
function v13PlanRefreshRows(){
  ...
  (state.stocks||[]).forEach(stock=>{
    normalizeStockAnalysis(stock);
    const plans=v13DisplayActivePlans(stock.plans).filter(PlanV2.isLegacyPricePlan);
    const seen=new Set();
    plans.forEach(plan=>{
...
function v13ValidatePlanRefreshPayload(stock,payload){
  ...
  if(!PlanV2.isLegacyPricePlan(data))errors.push('此导入入口仅支持价格计划。');
  ...
  plans.forEach((plan,index)=>{
    ...
    if(!PlanV2.isLegacyPricePlan(plan))errors.push(`${prefix} 不支持该 planMode。`);
...
function v13NormalizeImportedRefreshPlan(stock,plan,batchId){
  PlanV2.assertLegacyPricePlan(plan);
...
function render(){
  if(typeof parkMaintenanceControls==='function')parkMaintenanceControls();
  const d=new Date();
  ...
  document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active',t.dataset.tab===primaryTabFor(currentTab)));
  ...
  const syncBar=document.querySelector('.universe-cloud-bar');
  ...
  else if(currentTab==='tools')renderSimplifiedTools();
  else if(currentTab==='more')renderMore();
  else if(currentTab==='plans')renderTargets(true);
  else if(['targets','holding','etf','watching'].includes(currentTab)){if(currentTab!=='targets')targetFilter=ManagementCategory.session(currentTab);renderTargets();}
  else renderTable();
  if(typeof syncStorageMaintenanceUi==='function')syncStorageMaintenanceUi();
}
function homepageAttentionPanel(model){
  const priorityLabels={critical:'关键',high:'优先',medium:'关注'};
  const cards=model.items.map(item=>`<article class="card homepage-attention-card" data-home-attention-item data-priority="${esc(item.priority)}" style="margin-bottom:10px;border-left:3px solid ${item.priority==='medium'?'var(--gold)':'var(--seal)'};overflow-wrap:anywhere"><div class="card-title">${esc(item.name)} <span class="muted" style="float:right;font-weight:400">${priorityLabels[item.priority]}</span></div><div class="text" style="max-width:none;font-size:18px;font-weight:700">${esc(item.title)}</div><div class="card-note" style="margin-top:6px">${esc(item.summary)}</div>${item.secondary?`<div class="card-note" style="margin-top:5px">同时关注：${esc(item.secondary)}</div>`:''}<div class="modal-actions" style="justify-content:flex-start;margin-top:10px"><button class="btn ghost small" type="button" data-home-attention-action="${esc(item.cta.action)}" data-stock-id="${esc(item.cta.stockId||item.stockId||'')}">${esc(item.cta.label)}</button></div></article>`).join('');
  return `<section data-home-attention aria-label="今日关注"><h2 style="font-size:20px;margin:0 0 12px">今日关注</h2>${cards||`<div class="card" data-home-quiet><div class="text" style="max-width:none">${esc(model.quietText)}</div></div>`}${model.items.length?'<p class="card-note">其他标的暂无需要优先关注的变化。</p>':''}</section>`;
}
function renderDashboard(){
  const summary=document.getElementById('summary');
  const main=document.getElementById('main');
  if(!state.stocks.length){
    summary.innerHTML='暂无数据 · 更多 → 备份与恢复';
    main.innerHTML='<div class="hint"><b>尚未导入组合数据。</b> 请进入「更多 → 备份与恢复」，选择之前导出的投资作战手册 JSON；程序不会再内置或自动恢复默认组合。</div><div class="empty">导入后这里会显示值得关注的风险、计划变化和行情数据问题。</div>';
    return;
  }
  ...
  main.querySelectorAll('[data-home-attention-action]').forEach(button=>button.addEventListener('click',()=>{
    ...
    if(action==='discussion'){openStockDetail(id,'ai',{sourceContext:attentionSources.get(String(id))||null});}
      ...
      const dataPanel=document.querySelector('[data-technical-layer="status"]');
      if(dataPanel){dataPanel.open=true;dataPanel.scrollIntoView({block:'start'})}
    }
    else if(action==='sync')window.openUniverseSyncSettings();
  }));
}
...
function planLatestAction(s){
  const cp=getComparablePrice(s);
  const plans=v13DisplayActivePlans(s.plans).filter(PlanV2.isLegacyPricePlan).map(p=>({p,g:planGap(cp,p.triggerPrice,p.action,p.triggerDirection),validity:v13DerivedPlanValidity(s,p)})).filter(x=>Number(x.p&&x.p.triggerPrice)>0);
...
function nearestPlanForAction(stock,action){
  ...
  const rows=v13DisplayActivePlans(stock.plans)
    .filter(p=>PlanV2.isLegacyPricePlan(p)&&(action==='buy'?['buy','add'].includes(p.action):['sell','reduce'].includes(p.action))&&Number(p.triggerPrice)>0)
...
function collectPlans(capPct){
  const originals=(planEditorOriginalPlans||[]).map(plan=>PlanV2.normalizePlan(plan)),originalById=new Map(originals.map(plan=>[plan.id,plan]));
  const editable=plan=>PlanV2.isLegacyPricePlan(plan)&&plan.status==='active'&&['buy','add','sell','reduce'].includes(plan.action)&&(formType!=='watching'||!['sell','reduce'].includes(plan.action));
  const editedRows=[...tempBuy,...(formType==='watching'?[]:tempSell)],edits=new Map(),created=[];
  const positive=value=>Number.isFinite(Number(value))&&Number(value)>0;
  for(const row of editedRows){
    PlanV2.assertLegacyPricePlan(row);
    const existing=originalById.get(row.id),note=String(row.note||'').trim();
    if(row.id&&(!existing||!editable(existing)||edits.has(row.id)))throw new Error('计划编辑目标无效或重复。');
    if(existing){
      const patch={note};
      for(const field of ['triggerPrice','quantity']){
        const value=row[field]===null||row[field]===''?null:Number(row[field]);
        if(value===existing[field])continue;
        if(!positive(value))throw new Error('修改的计划价格和数量必须为正数。');
        patch[field]=value;
      }
      if(row.action!==existing.action)throw new Error('普通编辑器不支持更改计划动作。');
      edits.set(existing.id,PlanV2.applyAuthoritativeEdit(existing,patch));
    }else{
      if(!positive(row.triggerPrice)||!positive(row.quantity))throw new Error('新计划的价格和数量必须为正数。');
      created.push(PlanV2.createPlan({action:row.action,triggerPrice:Number(row.triggerPrice),triggerDirection:row.triggerDirection,quantity:Number(row.quantity),note,allocationConstraint:{maxPositionPct:Number(capPct)>0?Number(capPct):null,targetWeightRange:null}},{source:'manual'}));
    }
  }
  const result=originals.map(plan=>!editable(plan)?plan:(edits.get(plan.id)||PlanV2.terminatePlan(plan,'cancelled',{reason:'用户在普通计划编辑器中取消'}))).concat(created);
...
function nearestDetailPlan(stock){
  const cp=getComparablePrice(stock);
  const rows=v13DisplayActivePlans(stock.plans).filter(PlanV2.isLegacyPricePlan).map(p=>({p,g:planGap(cp,p.triggerPrice,p.action,p.triggerDirection)})).filter(x=>Number(x.p&&x.p.triggerPrice)>0);
  if(!rows.length)return null;
  rows.sort((a,b)=>{
    const av=a.g?(a.g.triggered?-1000+a.g.absPct:a.g.absPct):9999;
    const bv=b.g?(b.g.triggered?-1000+b.g.absPct:b.g.absPct):9999;
...
function positionPlanPanel(stock){
  ...
  const allActiveRows=activePlans.map(p=>!PlanV2.isLegacyPricePlan(p)?`状态观察计划（只读） · ${p.note||'—'}`:`${p.type||p.action||'plan'} ${fmtMaybe(p.triggerPrice!==undefined?p.triggerPrice:p.price)} · ${p.summary||p.actionHint||p.note||'—'}`);
...
function allocationDecisionContext(stock){
  ...
  return {
    ...
    buyPlans:(stock.plans||[]).filter(p=>PlanV2.isLegacyPricePlan(p)&&(p.action||'buy')==='buy'),
    sellPlans:(stock.plans||[]).filter(p=>PlanV2.isLegacyPricePlan(p)&&p.action==='sell'),
...
function technicalViewUx(){
  return window.TechnicalViewUx||{
    ...
    taskStatusPresentation:()=>({kind:'unknown',label:'暂无可靠任务状态',result:'暂无可靠记录',current:false}),
    technicalReviewBasisNotice:()=>'',
...
function technicalDataStatusLayer(stock,review,td,freshness){
  ...
  const taskView=technicalViewUx().taskStatusPresentation(task);
...
function aiDiscussionPromptText(stock){
  ...
  (stock.plans||[]).map(plan=>PlanV2.normalizePlan(plan)).filter(PlanV2.isLegacyPricePlan).forEach(p=>{
...
function copyAiDiscussionPrompt(){
  const stock=state.stocks.find(x=>x.id===detailStockId);
  if(!stock)return;
  startStockDiscussion(stock);
}
function formatPlanForDiscussion(stock,p){
  if(!PlanV2.isLegacyPricePlan(p))return '- 观察计划（只读）：'+(PlanV2.hasWatchDefinition(p)?`${p.name}；复核方向：${PlanV2.REVIEW_ACTION_LABELS[p.reviewAction]}；进入观察：${p.entryConditions.join('；')}；进一步确认：${p.confirmationConditions.join('；')}；失效条件：${p.invalidationConditions.join('；')}`:String(p.note||''));
...
function fullAddDiscussionPromptText(stock){
  ...
  const buyPlans=(stock.plans||[]).filter(p=>PlanV2.isLegacyPricePlan(p)&&(p.action||'buy')==='buy');
  const sellPlans=(stock.plans||[]).filter(p=>PlanV2.isLegacyPricePlan(p)&&p.action==='sell');
  const review=normalizeAiReviews(stock.aiReviews).comprehensiveReview||null;
  const sentiment=sentimentReviewContext(stock);
...
function normalizeImportedTradePlan(parsed,stock){
  ...
  PlanV2.assertLegacyPricePlan(parsed);PlanV2.assertLegacyPricePlan(src);
  const items=Array.isArray(src.planItems)?src.planItems:[];items.forEach(PlanV2.assertLegacyPricePlan);
...
function tradePlanToPlans(tradePlan,stock){
  PlanV2.assertLegacyPricePlan(tradePlan);(tradePlan.planItems||[]).forEach(PlanV2.assertLegacyPricePlan);
...
function renderAdvancedAnalysisOverview(mountId='main'){
  ...
  if(mountId==='main')document.getElementById('summary').innerHTML=`分析总览 <strong>${rows.length}</strong> / ${state.stocks.length} 只 · 平均分 <strong>${rows.length?fmt(rows.reduce((sum,s)=>sum+(Number(s.analysisScore)||0),0)/rows.length,1):'—'}</strong>`;
  const main=document.getElementById(mountId);
  ...
  document.getElementById('analysisSort').addEventListener('change',e=>{analysisSortMode=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisRole').addEventListener('change',e=>{analysisRoleFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisStatus').addEventListener('change',e=>{analysisStatusFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisRisk').addEventListener('change',e=>{analysisRiskFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisScoreRange').addEventListener('change',e=>{analysisScoreFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisAction').addEventListener('change',e=>{analysisActionFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisCapital').addEventListener('change',e=>{analysisCapitalFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisExecution').addEventListener('change',e=>{analysisExecutionFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisFreshness').addEventListener('change',e=>{analysisFreshnessFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisTechnicalStatus').addEventListener('change',e=>{analysisTechnicalStatusFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisValuationStatus').addEventListener('change',e=>{analysisValuationStatusFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisValuationMissing').addEventListener('change',e=>{analysisValuationMissingFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisFinancialStatus').addEventListener('change',e=>{analysisFinancialStatusFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
  document.getElementById('analysisFinancialMissing').addEventListener('change',e=>{analysisFinancialMissingFilter=e.target.value;renderAdvancedAnalysisOverview(mountId)});
...
function openStockDetail(id,workspace='',options={}){
  ...
  detailStockId=id;
  detailSubView='';
  setDetailWorkspace(workspace||'ai',{persist:false});
  render();
}
...
function v13CurrentActivePlanSummaryPanel(stock){
  ...
  const row=plan=>{
    if(PlanV2.hasWatchDefinition(plan)&&window.StateWatchWorkflow)return window.StateWatchWorkflow.card(plan,stock.id,{runtimeHtml:window.PlanRuntimeUI?window.PlanRuntimeUI.card(plan,stock,state):''});
...
function v13PlanCenterSummary(stock){
  ...
  const groupHtml=group=>`<section class="card" style="background:rgba(255,255,255,.58)"><div class="card-title">${esc(labels[group.key])}</div>${group.items.map(raw=>{const plan=PlanV2.normalizePlan(raw);if(PlanV2.hasWatchDefinition(plan)&&window.StateWatchWorkflow)return window.StateWatchWorkflow.card(plan,stock.id,{runtimeHtml:window.PlanRuntimeUI?window.PlanRuntimeUI.card(plan,stock,state):''});const view=v13DerivedPlanValidity(stock,plan),date=plan.lastReviewedAt?`最后复核 ${plan.lastReviewedAt.slice(0,10)}`:(plan.source==='migrated_legacy'?'历史计划，日期未知':'尚未复核'),next=plan.nextReviewDate?` · 下次复核 ${plan.nextReviewDate}`:'';return `<div class="trig-row ${PlanV2.isLegacyPricePlan(plan)?(group.key==='sell'?'sell':'buy'):''}"><div class="trig-name">${esc(businessLabels.get(plan.id)||`${labels[group.key]}计划`)} · ${v13PlanTriggerText(plan)} <span class="chip ${view.label==='有效'?'role':'warn'}">${esc(view.label)}</span></div><div class="trig-dist">${v13PlanQuantityText(plan)}</div><div class="trig-desc">${esc(v13PlanCenterShortText(plan))}<div class="card-note">${esc(date+next)}</div>${view.reviewRequired?`<button class="btn small" type="button" data-plan-reconfirm-id="${esc(plan.id)}">确认继续有效</button>`:''}</div></div>`}).join('')}</section>`;
...
function v13PlanCenterDetailSections(stock){
  ...
  return plans.map(raw=>{
    const plan=PlanV2.normalizePlan(raw);if(PlanV2.hasWatchDefinition(plan)&&window.StateWatchWorkflow)return window.StateWatchWorkflow.card(plan,stock.id,{editable:false,runtimeHtml:window.PlanRuntimeUI?window.PlanRuntimeUI.card(plan,stock,state):''});const category=v13PlanDisplayCategory(plan),view=v13DerivedPlanValidity(stock,plan),conditionText=PlanV2.CONDITION_CATEGORIES.flatMap(key=>plan.conditions[key].map(item=>item.text)).filter(Boolean).join('；')||'未记录',reviewDate=plan.lastReviewedAt?plan.lastReviewedAt.slice(0,10):(plan.source==='migrated_legacy'?'历史计划，日期未知':'尚未复核');
...
async function reconfirmPlanFromCenter(planId){
  const stock=state.stocks.find(item=>item.id===detailStockId),sourcePlan=stock&&(stock.plans||[]).find(plan=>plan.id===planId);if(!stock||!sourcePlan)return;if(!PlanV2.isLegacyPricePlan(sourcePlan)){alert('状态观察计划暂不支持在此处编辑。');return}if(!confirm('确认已人工复核该计划，并继续将其视为有效计划？\n\n这会记录本次复核日期，并安排 30 天后的下次复核。'))return;
...
function planWorkspacePanel(stock){
  ...
  const readiness=planGenerationGateResult(stock);
  const discussionPlanPanel=`<div class="card" style="margin-bottom:12px;border-left:4px solid var(--teal)"><div class="card-title">整理正式计划</div><div class="card-note">可直接根据当前 AI 对话整理计划；程序只用持仓和现有正式计划做安全绑定，不要求先保存讨论结论。</div><div class="modal-actions" style="justify-content:flex-start;margin-top:10px"><button class="btn small" type="button" data-detail-action="prepare-discussion-plan">整理计划</button><button class="btn ghost small" type="button" data-detail-action="import-discussion-plan">导入计划</button></div></div>`;
  ...
  return `<div class="card"><div class="card-title">当前计划</div><div class="actions"><button class="btn small" data-detail-action="view-plan-center" type="button">转到计划中心</button><button class="btn ghost small" data-workspace="operation" type="button">录入操作结果</button></div></div>${v13CurrentActivePlanSummaryPanel(stock)||'<div class="card">尚无当前计划，可到计划中心创建。</div>'}<details class="card"><summary>计划整理与高级操作</summary>${discussionPlanPanel}${planGenerationReadinessCard(stock)}${refreshPanel}${v13StockEventCompactPanel(stock)}${planUpdate}${planSummary}${workspaceDetails('查看详细计划',v13PlanCenterDetailSections(stock))}${workspaceDetails('查看仓位与计划详情',positionPlanPanel(stock))}</details>`;
}
...
function longTermWorkspacePanel(stock){
  ...
  return `${longTermAiStatusPanel(stock)}${summary}${workspaceDetails('查看长期逻辑详情',body)}${workspaceDetails('同步与版本',typeof analysisSyncStatusPanel==='function'?analysisSyncStatusPanel(stock):'')}`;
}
...
function navigateDiscussionWorkspace(stock,workspace){
  const allowed=['news','fundamental','longterm','valuation','technical','plan','research'];
...
function toggleDiscussionHistory(stock){discussionHistoryVisibility.add(discussionStockKey(stock));navigateDiscussionWorkspace(stock,'history')}
...
function discussionProgramPriceReferences(stock){
  if(!stock)return '';
  const rows=arrSafe(stock.plans).filter(plan=>plan&&plan.status==='active'&&['active','needs_review'].includes(plan.validityStatus||'active')&&Number(plan.triggerPrice)>0).map(plan=>({kind:['buy','add'].includes(plan.action)?'add':(['sell','reduce'].includes(plan.action)?'reduce':'other'),price:Number(plan.triggerPrice)}));
  const add=rows.filter(row=>row.kind==='add').map(row=>row.price),reduce=rows.filter(row=>row.kind==='reduce').map(row=>row.price),line=(label,prices)=>prices.length?`<div class="discussion-program-reference"><span>${esc(label)}</span><b>${prices.map(value=>esc(String(value))).join(' / ')}</b></div>`:'';
  return line('计划增加仓位参考',add)+line('计划减仓参考',reduce);
}
function discussionStateCard(item,label,stock=null){
  if(!item)return '';
  const list=(title,rows)=>`<div><div class="card-title">${esc(title)}</div>${rows&&rows.length?`<ul class="discussion-compact-list">${rows.map(row=>`<li>${esc(formatChineseText(row))}</li>`).join('')}</ul>`:'<div class="card-note">无</div>'}</div>`;
  const timeline=stock?[...arrSafe(stock.discussionState?.history),stock.discussionState?.current].filter(Boolean):[],itemIndex=timeline.findIndex(row=>row.stateId===item.stateId);
  const diagnostics=window.DiscussionStateContract.renderDiagnostics(item,itemIndex>0?timeline[itemIndex-1]:null);
  const entryCard=window.DiscussionWorkbench.EntryDecision.render(item,stock?stock.shares:item.references?.holding?.shares,esc);
  const meta=`<div class="discussion-state-head"><span class="chip tag">${esc(label)}</span><span>${esc(item.confirmedAt?new Date(item.confirmedAt).toLocaleString('zh-CN'):(item.confirmedDate||'—'))} · 技术锚点 ${esc(item.technicalAsOf||'—')}</span></div>`;
  if(!item.actionAssessment||!item.trendAssessment||!Array.isArray(item.structureAssessment))return `<article class="discussion-state-card discussion-state-legacy">${meta}${diagnostics}<h3>${esc(formatChineseText(item.stage||'已确认'))}</h3><p>${esc(formatChineseText(item.summary||'—'))}</p><div class="discussion-state-grid">${list('关键变化',item.keyChanges)}${list('风险',item.risks)}${list('观察点',item.watchPoints)}</div><div class="card-note"><b>与计划关系：</b>${esc(formatChineseText(item.planRelation||'未说明'))} · 置信度 ${esc(zhConfidence(item.confidence))}</div></article>`;
  const actionLabels={risk_control:'风险控制',reduce_review:'减仓复核',hold_watch:'持有观察',wait_confirmation:'等待确认',add_review:'加仓复核',entry_review:'建仓复核',no_action:'暂不操作'},priorityLabels={high:'高优先级',medium:'中优先级',low:'低优先级'},attentionLabels={normal:'普通观察',focused:'重点观察',window:'临近窗口'},trendLabels={uptrend:'上升',downtrend:'下降',sideways:'震荡',recovery:'修复',rebound:'反弹',unclear:'不明确'},typeLabels={top:'顶部结构',bottom:'底部结构',breakout:'突破结构',pullback:'回踩结构',recovery:'修复结构',consolidation:'整理结构',none:'暂无明确结构',unclear:'结构不明确'},statusLabels={forming:'形成中',confirmed:'已确认',valid:'仍有效',broken:'已破坏',unclear:'不明确'},sourceLabels={program:'程序事实',external_software:'外部软件',ai_chart_judgment:'AI图形判断',user_provided:'用户提供'};
  const trendRows=[`整体：${trendLabels[item.trendAssessment.overall]||'不明确'}`,...arrSafe(item.trendAssessment.timeframes).map(row=>`${row.timeframe}：${trendLabels[row.status]||'不明确'}｜${row.explanation}`)],structureRows=arrSafe(item.structureAssessment).map(row=>`${row.timeframe}：${typeLabels[row.type]||'结构不明确'}${statusLabels[row.status]||'不明确'}｜${sourceLabels[row.source]||'来源未说明'}${row.sourceAsOf?` ${row.sourceAsOf}`:''}｜${row.shortReason}`),decision=item.userDecision;
  if(!decision)return `<article class="discussion-state-card discussion-decision-card discussion-state-v2 ${item.actionAssessment.priority==='high'?'is-high':''}">${meta}${diagnostics}<div class="discussion-decision-badges"><span class="discussion-primary-badge">${esc(attentionLabels[item.attentionLevel]||'普通观察')}</span><span class="discussion-priority-badge">${esc(priorityLabels[item.actionAssessment.priority]||'低优先级')}</span></div><div class="discussion-action-label"><span>操作倾向</span><strong>${esc(actionLabels[item.actionAssessment.category]||'暂不操作')}</strong></div><h3>${esc(formatChineseText(item.actionAssessment.headline))}</h3><div class="discussion-decision-core"><div><div class="card-title">趋势</div><div class="discussion-inline-rows">${trendRows.map(row=>`<span>${esc(formatChineseText(row))}</span>`).join('')}</div></div><div><div class="card-title">结构</div><div class="discussion-inline-rows">${structureRows.length?structureRows.map(row=>`<span>${esc(formatChineseText(row))}</span>`).join(''):'<span>暂无明确结构</span>'}</div></div>${list('当前重点',item.focusPoints)}</div><div class="discussion-plan-relation"><b>与计划关系</b><span>${esc(formatChineseText(item.planRelation&&item.planRelation.summary||'未说明'))}</span></div><details class="discussion-state-details"><summary>查看完整结论</summary><div class="discussion-state-details-body"><p>${esc(formatChineseText(item.summary||'—'))}</p><div class="discussion-state-grid">${list('判断依据',item.actionAssessment.reasons)}${list('升级条件',item.actionAssessment.upgradeConditions)}${list('降级条件',item.actionAssessment.downgradeConditions)}${list('关键变化',item.keyChanges)}${list('风险',item.risks)}${list('完整观察点',item.watchPoints)}</div><div class="card-note">阶段：${esc(formatChineseText(item.stage||'已确认'))} · 置信度 ${esc(zhConfidence(item.confidence))}</div></div></details></article>`;
  const zeroPosition=item.references?.holding?.shares===0,warningItems=arrSafe(decision.warning&&decision.warning.items),riskLabels={stock:'风险主要来自个股',market:'风险主要来自大盘',both:'个股与大盘风险并存',unclear:'风险来源仍需确认'},section=(title,summary,className='')=>`<div class="discussion-user-section ${className}"><div class="card-title">${esc(title)}</div><p>${esc(formatChineseText(summary||'—'))}</p></div>`,takeProfit=section('止盈',decision.takeProfit.summary),stopLoss=section('止损',decision.stopLoss.summary),programReferences=discussionProgramPriceReferences(stock);
  return `<article class="discussion-state-card discussion-user-decision-card ${item.actionAssessment.priority==='high'?'is-high':''}">${meta}${entryCard}${diagnostics}${entryCard?'<details class="discussion-original-decision"><summary>原始 AI 表述</summary>':''}<section class="discussion-user-headline"><div class="card-title">当前结论</div><h3>${esc(formatChineseText(decision.headline))}</h3>${riskLabels[decision.riskSource]?`<span class="discussion-risk-source">${esc(riskLabels[decision.riskSource])}</span>`:''}</section><div class="discussion-user-grid">${section('持仓判断',decision.holding.summary)}${section('仓位方向',decision.positionDirection.summary,'is-position')}${section(zeroPosition?'如果想建仓':'如果想加仓',decision.addAssessment.summary,'is-add')}${entryCard?`<div class="discussion-profit-loss">${takeProfit}${stopLoss}</div></div></details><div class="discussion-user-grid">`:''}<div class="discussion-user-section is-warning"><div class="card-title">需要警惕</div><p>${esc(formatChineseText(decision.warning.summary||'—'))}</p>${warningItems.length?`<ul class="discussion-compact-list">${warningItems.map(row=>`<li>${esc(formatChineseText(row))}</li>`).join('')}</ul>`:''}</div>${!entryCard&&(takeProfit||stopLoss)?`<div class="discussion-profit-loss">${takeProfit}${stopLoss}</div>`:''}${programReferences?`<div class="discussion-program-references">${entryCard?'<strong>既有 Plan · 程序事实</strong><span class="discussion-plan-separation">独立于当前建仓判断</span>':''}${programReferences}${entryCard?`<span>${esc(arrSafe(stock?.plans).filter(p=>p.status==='active').map(p=>p.validityStatus||'active').join(' / '))}</span>${arrSafe(stock?.plans).some(p=>p.status==='active'&&p.validityStatus==='needs_review')?'<p class="discussion-plan-review-note">既有计划待复核，不作为当前建仓路径的唯一条件。</p>':''}`:''}</div>`:''}</div><details class="discussion-state-details discussion-evidence"><summary>判断依据</summary><div class="discussion-state-details-body"><div class="discussion-decision-badges"><span class="discussion-primary-badge">${esc(attentionLabels[item.attentionLevel]||'普通观察')}</span><span class="discussion-priority-badge">${esc(priorityLabels[item.actionAssessment.priority]||'低优先级')}</span><span class="discussion-priority-badge">${esc(actionLabels[item.actionAssessment.category]||'暂不操作')}</span></div><p>${esc(formatChineseText(item.summary||'—'))}</p><div class="discussion-decision-core"><div><div class="card-title">趋势</div><div class="discussion-inline-rows">${trendRows.map(row=>`<span>${esc(formatChineseText(row))}</span>`).join('')}</div></div><div><div class="card-title">结构</div><div class="discussion-inline-rows">${structureRows.length?structureRows.map(row=>`<span>${esc(formatChineseText(row))}</span>`).join(''):'<span>暂无明确结构</span>'}</div></div><div class="discussion-plan-relation"><b>与计划关系</b><span>${esc(formatChineseText(item.planRelation&&item.planRelation.summary||'未说明'))}</span></div></div><div class="discussion-state-grid">${list('判断理由',item.actionAssessment.reasons)}${list('升级条件',item.actionAssessment.upgradeConditions)}${list('降级条件',item.actionAssessment.downgradeConditions)}${list('关键变化',item.keyChanges)}${list('技术风险',item.risks)}${list('持续观察',item.watchPoints)}</div><div class="card-note">阶段：${esc(formatChineseText(item.stage||'已确认'))} · 置信度 ${esc(zhConfidence(item.confidence))}</div></div></details></article>`;
}
...
function previewDiscussionImport(){
  if(discussionImportSaving)return;
  discussionImportPreview=null;
  ...
  const date=window.DiscussionWorkbench.localCalendarDate(new Date(),{timeZone:'Asia/Shanghai'});message.textContent=result.message;preview.innerHTML=window.DiscussionStateContract.renderPreview(result,{holdingShares:current.context.currentFacts.holding.shares,technicalAsOf:prepared.technicalSnapshot?.anchorBar?.date,confirmedDate:date});
...
function openDiscussionPlanImportDialog(stock){
  if(!window.DiscussionPlanWorkflow)return alert('计划整理模块未加载。');
  try{
    const key=discussionStockKey(stock);let prepared=discussionPlanPreparedContexts.get(key),created=false;if(!prepared){prepared=window.DiscussionPlanWorkflow.prepare(stock);discussionPlanPreparedContexts.set(key,prepared);created=true}discussionPlanImportPreview=null;
    const el=ensureDiscussionPlanImportDialog();el.dataset.stockId=stock.id;document.getElementById('discussionPlanImportText').value='';document.getElementById('discussionPlanImportMessage').textContent=created?'已准备当前计划上下文。若已有旧 JSON，请重新整理计划；请完整复制 AI 返回的 JSON 代码块后导入。':'当前持仓和计划快照已锁定。请完整复制 AI 返回的 JSON 代码块，粘贴后先预览。';document.getElementById('discussionPlanImportPreview').innerHTML='';document.getElementById('discussionPlanImportConfirmBtn').disabled=true;document.getElementById('discussionPlanImportConfirmBtn').textContent='确认保存计划';el.classList.add('show');
...
function aiDiscussionWorkspacePanel(stock){
  ...
  const status=discussionStatusPresentation(stock),current=status.current,runtime=window.DiscussionWorkbench.buildContext(stock,discussionOptions()),readiness=window.DiscussionStateContract.assessTechnicalAnchorReadiness(runtime),blocking=!readiness.ready;
  const actions=`<div class="modal-actions discussion-actions" aria-label="讨论操作"><button class="btn small" data-detail-action="start-stock-discussion" type="button">开始讨论</button><button class="btn small" data-detail-action="prepare-discussion-archive" type="button">整理结论</button></div>`;
  const warning=blocking?'<div class="alert discussion-status-warning"><b>当前无法保存连续结论</b><span>缺少完整日K技术锚点。讨论可继续；补齐行情后重新开始讨论，才能保存。</span></div>':'';
...
function legacyStockWorkspaceTabs(stock){
...
function planListHtml(plans,type,cp){
  ...
  return `<div class="trig-list">${arr.map(p=>{if(PlanV2.hasWatchDefinition(p)&&window.StateWatchWorkflow)return window.StateWatchWorkflow.card(p,'',{editable:false});const g=PlanV2.isLegacyPricePlan(p)?planGap(cp,p.triggerPrice,p.action,p.triggerDirection):null;const tag={buy:'加仓',sell:'减仓',observe:'观察/防守',risk:'风险复核'}[type]||'计划',validity=v13DerivedPlanValidity({currentPrice:cp},p);const gap=!PlanV2.isLegacyPricePlan(p)?'仅供只读参考':g?(g.triggered?'价格已触发，条件待确认':(g.near?`接近目标 ${g.absPct.toFixed(1)}%`:`距目标 ${g.absPct.toFixed(1)}%`)):'方向待复核';return `<div class="trig-row ${PlanV2.isLegacyPricePlan(p)?(type==='sell'?'sell':'buy'):''}"><div class="trig-name">${tag} · ${v13PlanTriggerText(p)} · ${esc(validity.label)}</div><div class="trig-dist">${gap}</div><div class="trig-desc">${v13PlanQuantityText(p)}${p.note?' · '+esc(p.note):''}</div></div>`}).join('')}</div>`;
}
...
function planPromptList(plans,type){
  return (plans||[]).map(plan=>PlanV2.normalizePlan(plan)).filter(PlanV2.isLegacyPricePlan).filter(p=>type==='buy'?['buy','add'].includes(p.action):['sell','reduce'].includes(p.action)).map(p=>({triggerPrice:p.triggerPrice||'',quantity:p.quantity||'',note:p.note||'',triggerDirection:p.triggerDirection||'',validity:p.validityStatus==='active'?'有效':'需复核'}));
}
...
function planDraftValue(plan){if(!PlanV2.isLegacyPricePlan(plan))return {action:'状态观察计划（只读）',price:null,quantity:null,status:plan.status,reason:plan.note||'—'};return {action:plan&& (plan.action_type||plan.type||plan.action||'—'),price:plan&& (plan.trigger_price??plan.triggerPrice??plan.price??null),quantity:plan&& (plan.quantity??plan.shares??null),status:plan&& (plan.status||'active'),reason:plan&& (plan.reason||plan.summary||plan.note||'—')}}
...
function bindStockDetailActions(stock){
  ...
  mainEl.querySelectorAll('[data-workspace-tab]').forEach(btn=>{
    btn.addEventListener('click',e=>{
      ...
      const activeTab=document.getElementById(`workspace-tab-${workspacePrimaryKey(detailWorkspace)}`);
      if(activeTab)activeTab.focus({preventScroll:true});
    });
    btn.addEventListener('keydown',e=>{
      if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;
      e.preventDefault();
      const tabs=PRIMARY_WORKSPACE_META.map(item=>item.key);
...
function renderStockDetail(){
  ...
  document.getElementById('main').innerHTML=`<div data-v13-detail-anchor="overview"><div class="detail-compact-header"><div><h2>${esc(s.name)}</h2><div class="card-note">${esc(s.code)} · 现价 ${fmtMaybe(cp)} · 持仓 ${fmtInt(s.shares)}</div></div><button class="btn ghost small" id="backToListBtn" type="button">返回</button></div></div>${v13ReviewReturnBanner(s)}${stockWorkspaceTabs(s)}<details class="card"><summary>持仓与行情详情</summary>${detailHeroPanel(s,mv,actual,deviation)}</details>`;
