(function portfolioReviewUiModule(root){
  ...
  function ensureStyles(){
    ...
    const style=document.createElement('style');style.id='m05cStyles';style.textContent=`
      .m05c-switch{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0 14px}.m05c-switch .btn{min-height:44px}
      .m05c-mode-tabs{display:flex;gap:8px;margin-bottom:14px}.m05c-mode-tabs .btn{flex:1;min-height:44px}
  ...
  function ensureModal(){
    ...
    modal=document.createElement('div');modal.className='modal-bg import-layer';modal.id='portfolioReviewModal';modal.innerHTML=`<div class="modal"><h2>今日组合</h2><div class="m05c-switch"><button class="btn ghost" id="m05cTechnicalBtn" type="button">技术复核</button><button class="btn" type="button">今日组合</button></div><div class="m05c-mode-tabs"><button class="btn" id="m05cResultModeBtn" type="button">今日结果</button><button class="btn ghost" id="m05cGenerateModeBtn" type="button">生成复核</button></div><div id="m05cBody"></div><div class="modal-actions"><button class="btn ghost" id="m05cCloseBtn" type="button">关闭</button></div></div>`;
    ...
    document.getElementById('m05cResultModeBtn').addEventListener('click',()=>setMode('result'));document.getElementById('m05cGenerateModeBtn').addEventListener('click',()=>setMode('generate'));
    return modal;
  }
  function setMode(next){mode=next;const result=document.getElementById('m05cResultModeBtn'),generate=document.getElementById('m05cGenerateModeBtn');if(result){result.className=`btn${mode==='result'?'':' ghost'}`;generate.className=`btn${mode==='generate'?'':' ghost'}`;}renderBody()}
  ...
  function planLabel(value){return {approaching:'接近计划',triggered:'已到计划条件',invalidated:'计划需复核',not_close:'尚未接近',unclear:'不明确'}[value]||value}
  ...
  function stockRow(item,kind){
    ...
    return `<div class="m05c-result-row"><div class="m05c-row-label"><button type="button" data-m05c-symbol="${esc(symbol)}">${esc(name)}</button>${badge?`<span>${esc(badge)}</span>`:''}</div><div class="m05c-row-note">${esc(item.reason)}</div>${item.focus?`<div class="m05c-row-note">→ ${esc(item.focus)}</div>`:''}${item.planRelation?`<div class="m05c-row-note">计划：${esc(item.planRelation)}</div>`:''}</div>`;
  }
  ...
  function bullets(title,items){return items&&items.length?`<section class="m05c-section"><div class="m05c-section-title">${esc(title)}</div><ul class="m05c-bullets">${items.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></section>`:''}
  function renderSavedResult(){
    ...
    return `<div class="m05c-result-hero"><div class="m05c-result-meta"><span>${esc(review.reviewDate)}</span><span>${esc(stamp)}</span></div><div class="m05c-section-title">今日组合结论</div><div class="m05c-result-summary">${esc(review.summary)}</div><span class="m05c-risk">${esc(riskLabel(review.portfolioRiskLevel))}</span></div>${listSection('优先关注',review.priorityStocks,'priority')}${bullets('今日重点',review.todayFocus)}${listSection('风险关注',review.riskAttention,'risk')}${listSection('计划接近',review.planWatch,'plan')}${listSection('候选观察',review.candidateReview,'candidate')}${bullets('组合风险',review.portfolioRisks)}${bullets('数据限制',review.dataLimitations)}<div class="card-note">复核置信度：${esc(confidenceLabel(review.confidence))}</div>`;
  }
  ...
  function renderGenerate(){
    return `${renderSelection()}<div class="m05c-readiness" id="m05cReadiness"></div><div class="modal-actions" style="justify-content:flex-start;flex-wrap:wrap"><button class="btn ghost" id="m05cRefreshBtn" type="button">刷新结构化上下文</button><button class="btn" id="m05cCopyBtn" type="button">复制给 AI</button></div><details class="m05a-request-details" id="m05cRequestDetails"><summary>组合复核请求已准备</summary><textarea id="m05cRequestText" aria-label="组合复核请求" readonly style="min-height:190px"></textarea></details><div class="form-row"><label for="m05cResultText">粘贴 AI 结果</label><textarea id="m05cResultText" style="min-height:180px" placeholder='{"portfolioReview":{...}}'></textarea></div><div class="modal-actions"><button class="btn" id="m05cPreviewBtn" type="button">预览结果</button><button class="btn" id="m05cSaveBtn" type="button" disabled>保存复核</button></div><div id="m05cStatus" class="card-note" role="status" aria-live="polite" style="white-space:pre-line;margin-top:10px"></div><div id="m05cPreview"></div>`;
  }
  function renderBody(){
  ...
  function generateRequest(){
    ...
    try{currentContext=root.PortfolioReviewContext.buildPortfolioContext(selectedStocks(),{allStocks:stocks()});const request=root.PortfolioReviewContext.buildRequest(selectedStocks(),{allStocks:stocks(),reviewDate:currentContext.reviewDate,generatedAt:currentContext.generatedAt});requestField.value=request;const ready=currentContext.readiness,metrics=root.PortfolioReviewContext.requestMetrics(request);document.getElementById('m05cReadiness').innerHTML=`<span>${ready.stockCount}只股票</span><span>技术 ${ready.technical}</span><span>新闻 ${ready.news}</span><span>基本面 ${ready.fundamental}</span><span>估值 ${ready.valuation}</span>`;setStatus(`请求约 ${metrics.characters} 字符 / ${metrics.approxTokens} tokens。`);return request}catch(error){currentContext=null;requestField.value='';document.getElementById('m05cReadiness').innerHTML='';setStatus(error&&error.message||String(error),true);return ''}
  }
