...
function process(raw,options={}){
  ...
  function renderPreview(result,program={}){
    ...
    const entryCard=Workbench.EntryDecision.render(item,program.holdingShares,escapeHtml);
    return `<div class="discussion-import-preview"><div class="discussion-preview-anchor"><b>程序锚点</b><span>技术日 ${escapeHtml(technicalAsOf)}</span><span>确认日 ${escapeHtml(confirmedDate)}</span></div>${entryCard}${entryCard?'<details class="discussion-original-decision"><summary>原始 AI 表述</summary>':''}<div class="discussion-preview-decision"><b>当前结论</b><strong>${escapeHtml(decision.headline)}</strong><p><b>仓位方向：</b>${escapeHtml(decision.positionDirection.summary)}</p><p><b>${decision.holding.status==='not_applicable'?'如果想建仓':'如果想加仓'}：</b>${escapeHtml(decision.addAssessment.summary)}</p>${entryCard?'</div></details><div class="discussion-preview-risk">':''}<p><b>需要警惕：</b>${escapeHtml(decision.warning.summary)}</p>${entryCard&&decision.warning.items.length?list(decision.warning.items):''}</div><details><summary>判断依据</summary><dl><dt>操作复核</dt><dd>${escapeHtml(actionLabels[item.actionAssessment.category])} · ${escapeHtml(attentionLabels[item.attentionLevel])} · ${escapeHtml(priorityLabels[item.actionAssessment.priority])}</dd><dt>趋势</dt><dd>${list(trends)}</dd><dt>结构</dt><dd>${list(structures)}</dd><dt>当前重点</dt><dd>${list(item.focusPoints)}</dd><dt>与计划关系</dt><dd>${escapeHtml(item.planRelation.summary)}</dd><dt>核心结论</dt><dd>${escapeHtml(item.summary)}</dd><dt>置信度</dt><dd>${escapeHtml(item.confidence)}</dd></dl></details></div>`;
  }
